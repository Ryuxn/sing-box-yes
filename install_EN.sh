...
#System check
os_check() {
    LOGI "Detecting the current system..."
    ...
    LOGI "System detection is complete, the current system is: ${OS_RELEASE}"
}
...
arch_check() {
    ...
    LOGI "System architecture detection complete, the current system architecture is: ${OS_ARCH}"
}
...
config_check() {
    if [[ ! -f "${CONFIG_FILE_PATH}/config.json" ]]; then
        ...
        if [[ $? -ne 0 ]]; then
            LOGE "Configuration check failed, please check log"
        else
            LOGI "Configuration check success"
        fi
    fi
}
...
show_status() {
    ...
    0)
        ...
        show_enable_status
        LOGI "Config file path:${CONFIG_FILE_PATH}/config.json"
...
show_enable_status() {
    ...
    if [[ x"${temp}" == x"enabled" ]]; then
        echo -e "[INF] Whether sing-box starts automatically: ${green}Yes${plain}"
    else
        echo -e "[INF] Whether sing-box starts automatically: ${red}No${plain}"
    fi
}
...
create_or_delete_path() {
    ...
    if [[ $# -ne 1 ]]; then
        LOGE "Invalid input, should be one parameter, and can be 0 or 1"
        exit 1
    fi
    if [[ "$1" == "1" ]]; then
...
download_sing-box() {
    LOGD "Downloading sing-box..."
    os_check && arch_check && install_base
    if [[ $# -gt 1 ]]; then
        echo -e "${red}Invalid input, please check your input: $* ${plain}"
        exit 1
    elif [[ $# -eq 1 ]]; then
        SING_BOX_VERSION=$1
    ...
    LOGI "Will select the version to use: ${SING_BOX_VERSION}"
    ...
    if [[ $? -ne 0 ]]; then
        LOGE "Download sing-box failed, please check that your network are working properly and able to access github"
...
download_config() {
    LOGD "Downloading sing-box configuration template..."
    ...
    if [[ ! -f "${CONFIG_FILE_PATH}/config.json" ]]; then
        ...
        if [[ $? -ne 0 ]]; then
            LOGE "Failed to download sing-box configuration template, please check the network"
            exit 1
        else
            LOGI "Download sing-box configuration template success"
        fi
    else
        LOGI "${CONFIG_FILE_PATH} exist, no need to download again"
    fi
}
...
#backup config，this will be called when update sing-box
restore_config() {
    LOGD "Start restoring sing-box configuration files..."
...
install_sing-box() {
    ...
    if [[ ! -f "${DOWNLAOD_PATH}/sing-box-${SING_BOX_VERSION}-linux-${OS_ARCH}.tar.gz" ]]; then
        clear_sing_box
        LOGE "could not find sing-box packages, please check download path ${DOWNLAOD_PATH} whether it suceess"
        exit 1
    fi
    cd ${DOWNLAOD_PATH}
    ...
    if [[ $? -ne 0 ]]; then
        clear_sing_box
        LOGE "Unzip sing-box installation package failed, script will exit now"
        exit 1
    else
        LOGI "Unzip sing-box installation package success"
    fi
    ...
    LOGI "Installation of sing-box successful, and its started successfully"
}

#update sing-box
update_sing-box() {
    LOGD "Updating sing-box..."
    if [[ ! -f "${SERVICE_FILE_PATH}" ]]; then
        LOGE "This system does not have sing-box installed, please use the update command after installing sing-box"
        show_menu
    fi
...
uninstall_sing-box() {
    ...
    if [ $? -ne 0 ]; then
        LOGE "Uninstall sing-box failed, please check log"
        exit 1
    else
        LOGI "Uninstall sing-box success"
    fi
}

#install systemd service
install_systemd_service() {
    LOGD "Installing sing-box systemd service..."
    if [ -f "${SERVICE_FILE_PATH}" ]; then
...
start_sing-box() {
    if [ -f "${SERVICE_FILE_PATH}" ]; then
        ...
        LOGE "${SERVICE_FILE_PATH} does not exist, cannot start service"
        exit 1
    fi
}
...
restart_sing-box() {
    if [ -f "${SERVICE_FILE_PATH}" ]; then
        ...
        LOGE "${SERVICE_FILE_PATH} does not exist, cannot restart service"
        exit 1
    fi
}
...
#stop sing-box
stop_sing-box() {
    LOGD "Stopping sing-box service..."
    status_check
    if [ $? == ${SING_BOX_STATUS_NOT_INSTALL} ]; then
        LOGE "sing-box is not installed, cannot stop it"
        exit 1
        ...
        if ! systemctl stop sing-box; then
            LOGE "stop sing-box service failed, please check logs"
            exit 1
        fi
    fi
    LOGD "Stop sing-box service successful"
}
...
enable_sing-box() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "Set sing-box to start on system boot success"
    else
        LOGE "Failed to set sing-box to start automatically"
    fi
}
...
disable_sing-box() {
    ...
    if [[ $? == 0 ]]; then
        LOGI "Cancel sing-box auto-start success"
    else
        LOGE "Cancel sing-box auto-start failed"
    fi
}
...
show_log() {
    ...
    if [[ $? == ${SING_BOX_STATUS_NOT_RUNNING} ]]; then
        ...
        if [[ $? -ne 0 ]]; then
            ...
            local tempLog=''
            read -p "The log will be read from the log file, please enter the log file path, or press Enter to use the default path": tempLog
            if [[ -n ${tempLog} ]]; then
...
clear_log() {
    ...
    if [[ $# -gt 0 ]]; then
        ...
        if [[ ! -n ${filePath} ]]; then
            LOGI "The entered log file path is invalid, the default file path will be used"
    ...
    if [[ ${fileSize} -gt ${DEFAULT_LOG_FILE_DELETE_TRIGGER} ]]; then
        ...
        if [[ $? -ne 0 ]]; then
            ...
            LOGI "Clear sing-box log file success"
        fi
    else
        LOGI "The current log size is ${fileSize}M, which is less than ${DEFAULT_LOG_FILE_DELETE_TRIGGER}M, and will not be cleared"
    fi
}
...
enable_auto_clear_log() {
    ...
    if [[ ! -f ${filePath} ]]; then
        LOGE "${filePath} does not exist, Failed to set sing-box to clear logs regularly"
        exit 1
    fi
    ...
    LOGI "Set sing-box to clear logs success"
}
...
disable_auto_clear_log() {
    ...
    if [[ $? -ne 0 ]]; then
        LOGI "Failed to cancel sing-box scheduled log clearing"
    else
        LOGI "Cancel sing-box and clear the log success"
    fi
}
...
show_menu() {
    echo -e "
  ${green}sing-box-v${SING_BOX_YES_VERSION} Manage Scripts${plain}
...
————————————————
  ...
  ${green}C.${plain} Cancel sing-box auto-start
  ${green}D.${plain} Set sing-box to clear logs regularly & restart
  ${green}E.${plain} Cancel sing-box to clear logs regularly & restart
————————————————
  ${green}F.${plain} 一Open BBR
  ${green}G.${plain} 一Apply for an SSL certificate
 "
    show_status
    echo && read -p "Please enter a selection[0-G]:" num
